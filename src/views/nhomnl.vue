    ...
    <div class="container">
        ...
        <table class="data-table table table-dark table-striped">
            ...
            <tr v-for="(item, index) in displayedData" :key="item.id">
                <td>{{ item.id }}</td>
                <td>
                    <input v-if='availableAction()' v-model="item.tenNhom" type="text" class="form-control" id="tenNhom" @input='updateNhom(item.id, item.tenNhom)'>
                    <input v-else v-model="item.tenNhom" type="text" class="form-control" id="tenNhom" disabled>
                </td>
                <td width='20%' v-if='availableAction()'>
<!--                    <div class='align-items-center'>-->
<!--&lt;!&ndash;                        <button class='btn btn-warning m-1' onclick='document.getElementById("NNLForm").submit()'>Sửa</button>&ndash;&gt;-->
<!--&lt;!&ndash;                        <button class="btn btn-danger">Xóa</button>&ndash;&gt;-->
<!--                        -->
<!--                    </div>-->
                    Thay đổi tên ở cột 2 để thay đổi dữ liệu nhóm
                </td>
            </tr>
            </tbody>
        </table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            updateNhom(id, tenNhom) {
                const token = localStorage.getItem('token');
                const data = {
                    id: id,
                    tenNhom: tenNhom,
                }
                axios.put(`http://localhost:8081/api/nhom-nguyen-lieu/${id}`, data, {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`,
                    }
            ...
            addItem1() {
                window.location.href = '/nhom-nguyen-lieu/them';
            },
            addItem2() {
